...
enum NodeAction {
    ...
    Info,
    /// Check for Polygone updates via GitHub releases
    Update,
    /// Toggle CPU performance boost for the node process
    Boost,
    /// Show node performance diagnostics
    Status,
}
...
async fn cmd_node(action: NodeAction) -> anyhow::Result<()> {
    ...
    match action {
        NodeAction::Start { ram_mb, listen } => {
            ...
            if cfg.auto_update {
                println!("  🔄 Checking for updates (auto-update enabled)...");
                match boost::check_for_updates().await {
                    Ok(Some(new_ver)) => {
                        println!("  ✦ Update available: v{new_ver}");
                        println!("  → Applying update...");
                        if let Err(e) = boost::apply_update(&new_ver).await {
                            println!("  ⚠ Update failed: {e}");
                            println!("  → Continuing with current version");
                        } else {
                            println!("  ✓ Update applied! Please restart the node.");
                            std::process::exit(0);
                        }
                    }
                    Ok(None) => {
        ...
        NodeAction::Info => {
            ...
            println!("  Performance mode: {}", cfg.performance_mode);
        }
        NodeAction::Update => {
            println!("⬡ POLYGONE — Checking for updates");
            println!();
            match boost::check_for_updates().await {
                Ok(Some(new_ver)) => {
                    println!("  ✦ Update available: v{new_ver}");
                    println!();
                    boost::apply_update(&new_ver).await?;
                }
                Ok(None) => {
                    println!("  ✔ You are running the latest version: {}", crate::VERSION);
                }
                Err(e) => {
                    println!("  ✗ Could not check for updates: {e}");
                }
        ...
        NodeAction::Boost => {
            let cfg = NodeConfig::load();
            let new_state = !cfg.cpu_boost;
            println!("⬡ POLYGONE NODE — CPU Boost");
            println!();
            println!("  CPU boost: {} → {}", cfg.cpu_boost, new_state);
            boost::apply_cpu_boost(new_state);
            println!();
            if new_state {
                println!("  ✔ CPU boost enabled (performance governor, niceness -10)");
            } else {
                println!("  ✔ CPU boost disabled (ondemand governor, niceness 0)");
            }
        }
        NodeAction::Status => {
            let perf = boost::NodePerf::collect();
            println!("⬡ POLYGONE NODE STATUS");
            println!();
            println!("  {}", perf.describe());
...
fn cmd_tui(view: String) -> anyhow::Result<()> {
    use polygone::tui::views::View;
    let initial = match view.as_str() {
        "accueil"    | "home" | "dashboard" => View::Dashboard,
        "favoris"    | "favorites"         => View::Favorites,
        "services"   | "services"          => View::Services,
        "parametres" | "params" | "settings" => View::Settings,
        _                                    => View::Dashboard,
    };
